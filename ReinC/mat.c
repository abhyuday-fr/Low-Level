...
b32 matmul(matrix *out, const matrix *a, const matrix *b, b8 zero_out,
...
b32 reinforce_loss(matrix *out, const matrix *probs, const matrix *advantages) {
  ...
  if (out->rows != advantages->rows || out->cols != advantages->cols) {
    return false;
  }
...
b32 reinforce_add_grad(matrix *prob_grads, const matrix *probs,
  ...
  if (probs->rows != rt->rows || probs->cols != rt->cols) {
    return false;
  }
  if (grad->rows != probs->rows || grad->cols != probs->cols) {
    return false;
  }
  if (prob_grads->rows != probs->rows || prob_grads->cols != probs->cols) {
    return false;
  }
